...
SHEET = Path("docs/paper1/ABBILDUNGEN.md")
TOLERANCE = 1e-6
...
def main() -> int:
    ...
    for key in ("T1", "T2", "F1", "F2", "F3", "F4", "F5", "F6", "A1"):
        ...
        size = "{:.0f} kB".format(pdf.stat().st_size / 1024) if pdf.exists() else "fehlt"
        lines.append("| {} | {} | `{}` | {} |".format(key, size, pdf.as_posix(), SOURCES[key]))
    lines += ["", "## Geprueffte Zahlen", "",
              "| Abbildung | Groesse | gezeichnet | registriert | stimmt |", "|---|---|---|---|---|"]
    for c in checks:
        lines.append("| {} | {} | {:.6g} | {:.6g} | {} |".format(
            c["figure"], c["quantity"], c["drawn"], c["registered"], "ja" if c["agrees"] else "**nein**"))
    lines += ["", "Bildunterschriften stehen in `derive_surface/figures_p1.py` unter `CAPTIONS` und gehen von",
              "dort ins Manuskript.", ""]
    SHEET.write_text("\n".join(lines))
    print("\n".join(lines[-len(checks) - 4:]))
    print("\n{} Pruefungen, {} abweichend".format(len(checks), sum(1 for c in checks if not c["agrees"])))
