...
| Gesamtzahl Options-Zeilen über alle Underlyings 1 229 528 (ohne `currency`-Parameter abrufbar); BTC 309 385, ETH 811 068, HYPE 88 316 | ein Durchlauf für alle 12 Underlyings |
...
| `get_liquidation_history` meldet `count` 76, liefert aber je Seitengrösse andere Teilmengen (Seitengrösse 100 → 23 Auktionen) | Vereinigung über mehrere Seitengrössen, gemeldete vs gefundene Zahl im Datenstand; Chain-Events (`DutchAuction`) als Fallback |
| Vault-Wallets: Help-Center-Seite „Vault Smart Contracts“ nennt 202 Adressen (Bridges, Connectors, TSA-Tokens mehrerer Chains) | kuratierte Liste `docs/paper1/meta/vault_wallets.csv` mit Quelle je Adresse, Gegenprobe gegen die Wallets im Tape |
