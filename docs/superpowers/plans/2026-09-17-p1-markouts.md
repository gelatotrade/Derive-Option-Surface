...
**Goal:** For every fill of the pre-registered sample, compute the markouts over all horizons in three units (USDC, delta-neutral, vol points) with the mark paths (a), (b) and (c), attach cells, size and sweep flags, and store the result as one file. Inference (H1 to H4) follows in plan 3.

**Architecture:** One module `derive_surface/markouts.py` with small, individually tested functions (sample, state at fill time, path b per horizon, units, path a, settlement with VRP adjustment, sweeps) and an orchestration; CLI command `p1 markouts`. Path (b) uses `markpath.attach_svi`/`mark_from_svi` (plan 1).

**Tech Stack:** Python 3.9.6, numpy, pandas; pytest.

**Spec:** `docs/superpowers/specs/2026-09-17-adverse-selection-design.md`; **Pre-registration:** `docs/paper1/PRAEREGISTRIERUNG.md` (authoritative for every definition).

## Global Constraints

- All constraints from plan 1 apply (Python 3.9, no new dependencies, tests offline, commits local with trailer).
- Addendum 1 to the pre-registration is committed before task 1 (task 0). No markout number before this commit.
- Definitions exactly as in the pre-registration and addendum 1: maker direction s = −taker_side; MO = s·(M(t+τ) − P); delta-neutral with the forward of the SVI curve at both ends; vol points = 100·s·(IV_b(t+τ) − IV_fill(t)); path (b) by push time `block_ts`; horizons 60, 300, 1 800, 14 400, 86 400 s and settlement; exclusions: not settled, `pair_ok` false, last 30 min before expiry, no curve at fill time.
- Sample: BTC, ETH, HYPE; taker timestamps in [2024-01-11 00:00, cut-off] UTC; final cut-off 2026-09-30 08:00 UTC (`1_790_755_200_000`), pilot 2026-09-17 12:00 UTC (`1_789_646_400_000`).
- Output `data/p1/derived/markouts.parquet` (one row per sample fill) and `markouts_summary.json` (counts only: sample steps, coverage per path and horizon). No aggregated markout means in plan 2.

## Files

| File | Responsibility |
|---|---|
| `docs/paper1/PRAEREGISTRIERUNG.md` | Append addendum 1 |
| `derive_surface/markouts.py` | Sample, state, paths a/b/c, units, cells, sweeps, orchestration |
| `derive_surface/p1cli.py` | Subcommand `markouts` |
| `tests/test_p1_markouts.py` | Offline tests with synthetic, flat SVI curves |

---

### Task 0: Addendum 1 to the pre-registration

**Files:** Modify `docs/paper1/PRAEREGISTRIERUNG.md` (append at the end)

- [ ] **Step 1: Append the addendum**

```markdown

## Addendum 1 (2026-09-17, before the first markout computation)

1. **Delta-neutral markout:** S(t) and S(t+τ) are both the forward (`SVI_fwd`) of the SVI curve valid at the respective time. The original wording ("`index_price` of the fill, or `SVI_fwd` at time t+τ") would have carried the basis between index and forward, up to several 100 bp at long tenors, into the markout.
2. **Data beyond the cut-off:** Tape and SVI history are loaded up to 2026-10-01 09:00 UTC (cut-off + 25 h), so that fills in the last 24 h before the cut-off receive all horizons. The sample remains limited to taker timestamps ≤ 2026-09-30 08:00 UTC. Settlement markouts exist only for expiries up to the end of the data; a horizon that reaches or passes the expiry is not defined for path (b).
3. **Start of the tape:** The tape begins on 2023-12-06; the sample still begins on 2024-01-11.
4. **Path (a):** Mark of the first fill of the same instrument with a timestamp ≥ t+τ; the distance to the target time is stored alongside, so that analyses can be restricted to nearby fills (e.g. ≤ 10 % of τ or ≤ 5 min).
5. **Sweeps** are determined per (taker wallet, underlying): a fill belongs to a sweep if a 10-s window starting at a fill of the same taker contains at least 5 fills across at least 3 strikes and the fill lies inside it.
6. **Size flag:** "above the 90th percentile of size" refers to the notional (quantity × index) within the underlying over the whole sample.
```
...
**Interfaces:**
- Consumes: `markpath.attach_svi(rows, svi, at_ms, clock) -> DataFrame` (appends `svi_a … confidence, svi_block, svi_clock, svi_age_s`), `markpath.mark_from_svi(rows, at_ms, forward=None) -> Series`, `markpath.SVI_COLUMNS`; `chainfeeds.svi_vol`, `chainfeeds.load_feed`; `pricing.implied_vol`, `pricing.delta_of`, `pricing.YEAR`; `fills.parquet` from plan 1 (columns see plan 1, task 4); `ref/settlement_prices.parquet` (`currency, expiry, expiry_date, settlement_price`).
- Produces:
  - Constants `CORE`, `SAMPLE_START_MS`, `FINAL_CUTOFF_MS`, `PILOT_CUTOFF_MS`, `EXPIRY_BUFFER_MS`, `HORIZONS_S` (names `1m, 5m, 30m, 4h, 24h`), `DELTA_LABELS`, `TENOR_LABELS`
  - `sample_fills(fills, start_ms=SAMPLE_START_MS, cutoff_ms=FINAL_CUTOFF_MS, currencies=CORE) -> (DataFrame, list[tuple[str, int]])`
  - `flag_sweeps(fills, window_ms=10_000, min_fills=5, min_strikes=3) -> pd.Series[bool]` (index as in `fills`)
  - `state_at_fill(rows, svi) -> (DataFrame, int)`: additional columns `maker_side, fwd_t, iv_mark_t, iv_fill, mark_b_t, delta_t, abs_delta_pct, delta_bucket, tenor_days, tenor_bucket, svi_age_s_t`; second value = rows without a curve
  - `marks_b(state, svi, horizons=HORIZONS_S) -> DataFrame` with `mark_b_<h>, iv_b_<h>, fwd_b_<h>` (index as in `state`)
  - `add_markouts(state, marks, horizons=HORIZONS_S) -> DataFrame` with `mo_usd_<h>, mo_dn_<h>, mo_vol_<h>`
  - `add_path_a(state, fills, horizons=HORIZONS_S) -> DataFrame` with `mark_a_<h>, lag_a_<h>_s, mo_usd_a_<h>`
  - `add_settlement(state, settle) -> DataFrame` with `settlement_price, payoff, mo_set, mo_set_vrp`
  - `add_size_flag(state, q=0.9) -> DataFrame` with `size_above_p90`
...
Expected: FAIL with `ImportError: cannot import name 'markouts'`
...
### Task 2: CLI `p1 markouts` and pilot run
...
- Produces: `python3 -m derive_surface p1 markouts [--cutoff ISO]` (default: pilot cut-off) → `data/p1/derived/markouts.parquet`, `markouts_summary.json`.

- [ ] **Step 1: Add the test** (`tests/test_p1_cli.py`)
...
- [ ] **Step 2: Add the subcommand** (`derive_surface/p1cli.py`, after the `fills` parser and in the dispatch)
...
- [ ] **Step 3: Tests:** `python3 -m pytest -q` → all green

- [ ] **Step 4: Pilot run** (after plan 1, task 7)

Run: `python3 -m derive_surface p1 markouts --cutoff 2026-09-17T12:00:00Z`
Expected: `steps` with plausible decreases; `without_curve` small per underlying (fills before the first push of an expiry); `coverage` for `mo_usd_30m` close to `rows`, for `mo_set` only expired series. Do not print any means.
...
## Notes for the final run (after 2026-10-01 10:00 UTC)
...
Plan 3 (inference H1 to H4, net edge, robustness of the paths, path d with Tardis) builds on `markouts.parquet`.
