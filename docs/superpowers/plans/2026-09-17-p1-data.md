# Paper 1, Plan 1: Data Foundation and Counterparty Classes

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Secure the complete option tape (all 20 fields, both rows per fill), the onchain SVI history of BTC/ETH/HYPE and all reference data, pair maker and taker rows into fills, and give every fill a counterparty class, after the hypotheses have been pre-registered.

**Architecture:** Four focused modules in the existing package `derive_surface` (fulltape, chainfeeds, refdata, classify), one CLI entry point `python3 -m derive_surface p1 …`, one check script for the vol feeds. All downloads are resumable and write to disk chunk by chunk; all tests run offline against fake clients.

**Tech Stack:** Python 3.9.6 (system), numpy 2.0, pandas 2.3, pyarrow 21, requests; pytest. No new dependencies.

**Spec:** `docs/superpowers/specs/2026-09-17-adverse-selection-design.md` (in particular §2a with the findings of the data probe).

## Global Constraints

- Python 3.9.6: every new file starts with `from __future__ import annotations`; no `match`, no `zip(strict=…)`, no `X | Y` types outside annotations.
- No new dependencies beyond `requirements.txt`.
- Tests offline, no network; `python3 -m pytest -q` must be green before every commit (28 tests so far).
- Run everything from the repository root `~/Documents/Papers/Working Papers/Derive_Options/Derive-Option-Surface`; no `pip install -e` (pyproject requires ≥ 3.10).
- Data only under `data/p1/` (git-ignored). Do not delete anything that was not created in this session.
- Pilot cutoff `2026-09-17T12:00:00+00:00`; final cutoff `2026-09-30T08:00:00+00:00` (separate run later). **No** markout number is computed before the pre-registration is committed.
- Code, comments and commit messages in English; documents under `docs/paper1/` in German with umlauts.
- Commits only locally on branch `paper1-adverse-selection`; no push without the author's approval. Commit trailer: `Co-Authored-By: Claude Opus 5 (1M context) <noreply@anthropic.com>`.
- Vol-feed addresses (lowercase): BTC `0x388341d9e5a7d7d5accd738b2a31b0622e0c1b87`, ETH `0xb27cb6b08e6c298c8634d73d5f6649665e90d160`, HYPE `0x481916590863053ea2d8f21b64ee9429820512d1`; topic `VolDataUpdated` = `0x0b6ec9c174360425894fd5ff56d14f3450d70f2c9cde3a25983d652a72b84606`.
- Disk space: 15 GB free. Do not store the raw format of the chain logs, only decoded Parquet chunks.

## Files

| File | Responsibility |
|---|---|
| `derive_surface/fulltape.py` | Load the option tape in single-page windows, manifest, condense to Parquet |
| `derive_surface/chainfeeds.py` | RPC client, adaptive `eth_getLogs`, decoding of `VolDataUpdated`, chunk sync, compaction, SVI vol |
| `derive_surface/refdata.py` | Settlement prices, liquidations, maker programmes/scores, vaults, instrument fees, funding |
| `derive_surface/classify.py` | Pair fills, wallet-month statistics, class sets, classification, orchestration |
| `derive_surface/p1cli.py` | Subcommands `tape`, `volfeed`, `compact`, `ref`, `fills` |
| `derive_surface/__main__.py` | Routing of `p1` to `p1cli` |
| `scripts/p1_check_feeds.py` | Feed continuity (samples) and cross-check of SVI against the live mark |
...
| `tests/fixtures/vol_log_btc_44814062.json` | real log from 2026-09-17 (already present) |
| `docs/paper1/PRAEREGISTRIERUNG.md` | Hypotheses, tests, exclusions, fixed before any markout number |
| `docs/paper1/meta/vault_wallets.csv` | curated vault wallets with source |
| `docs/paper1/DATA_STATUS.md` | Counts and checks after the runs |
| `docs/paper1/feed_check.md` | Output of `scripts/p1_check_feeds.py` |

---

### Task 0: Branch, pre-registration, ignore rule

**Files:**
- Create: `docs/paper1/PRAEREGISTRIERUNG.md`
- Modify: `.gitignore` (line `data/p1/` is already appended)
- Add: `docs/superpowers/specs/2026-09-17-adverse-selection-design.md`, this plan, `tests/fixtures/vol_log_btc_44814062.json`

**Interfaces:** no code interfaces. Fixes the definitions that Plan 2 must implement exactly.

- [ ] **Step 1: Write the pre-registration**

File `docs/paper1/PRAEREGISTRIERUNG.md` with the following content:

> English translation. The binding text is the German original `docs/paper1/PRAEREGISTRIERUNG.md` as committed in `3fd9caa`; `docs/paper1/PREREGISTRATION.md` translates the full file including its addenda.
...
```bash
git add .gitignore docs/superpowers/specs/2026-09-17-adverse-selection-design.md docs/superpowers/plans/2026-09-17-p1-data.md docs/paper1/PRAEREGISTRIERUNG.md tests/fixtures/vol_log_btc_44814062.json
...
### Task 1: `fulltape.py`, option tape in single-page windows
...
**Interfaces:**
- Consumes: `derive_surface.api.OptionName.parse(name) -> OptionName` (fields `currency`, `expiry_ts`, `strike`, `option_type`); a client with `call(method: str, **params) -> dict` (like `DeriveClient`).
- Produces:
  ...
  - `download_tape(client, out_dir: Path, start_ms: int, end_ms: int, *, instrument_type: str = "option", workers: int = 6) -> dict` with keys `instrument_type, from_ms, to_ms, rows_in_windows, api_count, windows, leaves, finished_ms`
  - `condense(out_dir: Path, manifest: dict) -> pd.DataFrame` with columns `FIELDS + ["currency", "expiry", "strike", "option_type"]`
  - `write_tape(df: pd.DataFrame, tape_dir: Path) -> dict` writes `<CCY>_options_full.parquet` and `MANIFEST.json`
...
Expected: FAIL with `ImportError: cannot import name 'fulltape'`
...
### Task 2: `chainfeeds.py`, SVI history from Derive Chain
...
- Produces:
  - Constants `RPC_URL`, `VOL_DATA_UPDATED`, `VOL_FEEDS: dict[str, dict]` (keys `address`, `from_block`), `CHUNK_BLOCKS = 50_000`, `MAX_WINDOW = 50_000`, `LOG_LIMIT = 10_000`, `COLUMNS`
  - `class RpcError(RuntimeError)` with attribute `error: dict` and property `too_many_logs: bool`
  - `class ChainClient` with `call(method, params)`, `block_number() -> int`, `block_timestamp(n) -> int`, `get_logs(address: str | None, topic0, from_block, to_block) -> list[dict]`
  ...
  - `decode_vol_log(entry: dict) -> dict` (keys `block, log_index, expiry, feed_ts, svi_a, svi_b, svi_rho, svi_m, svi_sigma, svi_fwd, svi_ref_tau, confidence`)
  ...
  - `scan_emitters(client, blocks: list[int], span: int = 300) -> pd.DataFrame` (columns `sample_block, address, n, fwd, ts`)
...
Expected: FAIL with `ImportError: cannot import name 'chainfeeds'`
...
### Task 3: `refdata.py`, reference data
...
**Interfaces:**
- Consumes: client with `call(method, **params)`; `derive_surface.api.DeriveError` (for skipped score epochs).
- Produces:
  ...
  - `liquidations(client, page_sizes=(5, 20, 50, 100)) -> (auctions: DataFrame, bids: DataFrame, info: dict)`; `auctions` columns `auction_id, subaccount_id, start_timestamp, end_timestamp, auction_type, fee, tx_hash`; `bids` columns `auction_id, subaccount_id, timestamp, tx_hash, percent_liquidated, cash_received, discount_pnl, instruments`
  ...
  - `save_all(client, ref_dir: Path, currencies=OPTION_CURRENCIES) -> dict`; files `settlement_prices.parquet`, `liquidation_auctions.parquet`, `liquidation_bids.parquet`, `maker_programs.parquet`, `maker_scores.parquet`, `vault_statistics_<YYYYMMDD>.parquet`, `instrument_fees_<YYYYMMDD>.parquet`, `funding_history.parquet`, `REFDATA.json`
...
Expected: FAIL with `ImportError: cannot import name 'refdata'`
...
### Task 4: `classify.py`, pair fills and classify takers
...
**Interfaces:**
- Consumes: tape frame from `fulltape.condense` (columns `FIELDS + currency, expiry, strike, option_type`); `refdata` files `liquidation_auctions.parquet`, `liquidation_bids.parquet`, `maker_scores.parquet`.
- Produces:
  - `CLASS_ORDER = ["liquidation", "vault", "rfq", "dominant_maker", "mm_programme", "large", "other"]`, thresholds `DOMINANT_MAKER_SHARE = 0.80`, `DOMINANT_MIN_VOLUME_SHARE = 0.01`, `LARGE_TAKER_QUANTILE = 0.99`
  - `month_of(ts_ms) -> pd.Series` (`"YYYY-MM"`)
  - `pair_fills(tape) -> (fills, unpaired_rows)`; `fills` columns: `trade_id, ts, ts_maker, instrument_name, currency, expiry, strike, option_type, price, amount, mark_price, index_price, mark_price_maker_row, taker_side, taker_wallet, taker_sub, maker_wallet, maker_sub, rfq_id, tx_hash, tx_status, fee_taker, fee_maker, rebate_maker, rebate_taker, pnl_taker, pnl_maker, notional, pair_ok`
  ...
  - `load_vault_wallets(path) -> set[str]` (CSV columns `wallet, vault_name, source`)
  - `classify_fills(fills, *, liquidation_tx, vault_wallets, dominant, large, mm_lookup) -> DataFrame` (additionally `is_liquidation, taker_is_vault, maker_is_vault, is_rfq, taker_is_dominant_maker, maker_is_dominant_maker, taker_in_mm_programme, taker_is_large, taker_class, month`)
  - `build_fills(tape_dir, ref_dir, vault_csv, out_path) -> dict` (writes `fills.parquet` and `wallet_month_stats.parquet`)
...
### Task 5: CLI `p1`, feed check script

**Files:**
- Create: `derive_surface/p1cli.py`, `scripts/p1_check_feeds.py`
- Modify: `derive_surface/__main__.py` (start of `main`)
...
- Produces: `p1cli.to_ms(iso: str) -> int`, `p1cli.main(argv: list[str] | None) -> None`; commands `python3 -m derive_surface p1 tape --end ISO [--start ISO] [--workers N]`, `p1 volfeed [CCY…] --end ISO [--workers N] [--window N]`, `p1 compact [CCY…]`, `p1 ref`, `p1 fills [--vaults CSV]`; the script writes `docs/paper1/feed_check.md`.
...
Expected: FAIL with `ImportError: cannot import name 'p1cli'`
...
`derive_surface/__main__.py`: insert in `main` before `p = argparse.ArgumentParser(...)`:
...
def main() -> None:
    ...
    lines = ["# Vol-feed check", "",
             f"Run {time.strftime('%Y-%m-%d %H:%M UTC', time.gmtime())}, head block {head}, "
             f"{len(blocks)} samples of 200 blocks each from block 2 500 000, spaced 500 000 apart.", "",
             "## Continuity", "", "| Feed | Samples with events | first | last | Forward first → last |", "|---|---|---|---|---|"]
    ...
    lines += ["", "## Possible replacement feeds (unknown address with a forward within ±20 % of a core feed)", ""]
    lines += (["| Block | Core feed | Candidate |", "|---|---|---|"] + conflicts) if conflicts else ["None."]
    lines += ["", "## Cross-check of SVI against live mark IV", "",
              "| Underlying | Expiry | Options | max. |Δ IV| | Median |Δ IV| | SVI age (s) |", "|---|---|---|---|---|---|"]
    api = DeriveClient()
...
### Task 6: Curated vault wallets

**Files:**
- Create: `docs/paper1/meta/vault_wallets.csv`

**Interfaces:**
- Produces: CSV with header row `wallet,vault_name,source` (optionally further columns `chain_note,verified_on`), readable with `classify.load_vault_wallets`.

- [ ] **Step 1: Evaluate the sources**

From https://help.derive.xyz/en/articles/9351681-vault-smart-contracts (and linked articles in the collections “Vaults” and “Safe Harvest Vaults”), extract every table row with chain “Derive”: these are the TSA token contracts (Tokenized SubAccounts) on Derive Chain that appear as wallets on the tape. Do not include bridges, connectors or addresses on other chains. In addition, match the vault names from `get_vault_statistics` (10 vaults). Record every address with the URL it comes from.

- [ ] **Step 2: Cross-check against the tape**

After Task 7 Step 1 (tape loaded):
...
Expected: at least some of the addresses appear as takers with a high RFQ share (vault rolls run via RFQ auction). Addresses without a single tape hit stay in the list but are reported in the data status.
...
### Task 7: Pilot runs and data status

**Files:**
- Create: `docs/paper1/DATA_STATUS.md`, `docs/paper1/feed_check.md` (from the script)
- Data: `data/p1/…` (not versioned)

**Interfaces:**
- Consumes: all CLI commands from Task 5.
...
- [ ] **Step 1: Tape up to the pilot cutoff**

Run: `nohup caffeinate -dimsu python3 -m derive_surface p1 tape --end 2026-09-17T12:00:00 > data/p1/logs_tape.txt 2>&1 &` (run `mkdir -p data/p1` first)
Expected (after about 30 to 60 min): last line of the JSON with `rows_in_windows` equal to `api_count` (≈ 1 229 000), `files` with 12 underlyings. On a mismatch: repeat the run (the window files remain), then investigate the difference per day.

- [ ] **Step 2: Reference data**

Run: `python3 -m derive_surface p1 ref`
Expected: `settlement_prices` for 12 underlyings, `liquidations` with `reported_count` and `unique_auctions`, `maker_programmes.active_wallets` > 0, `funding_rows` ≈ 2 160.

- [ ] **Step 3: Calibrate the vol feed**

Run: `python3 -m derive_surface p1 volfeed BTC --end 2024-01-20T00:00:00 --workers 4`
Expected: about 13 chunks from block 2 400 000, a log line with `events/s`. Then the same sample for a late window: `python3 - <<'EOF'` with `sync_feed` on `to_block=44_000_000` for a test currency is not needed; instead read the throughput from the log and estimate the total duration for ~38 million events. If it exceeds 12 h, use `--workers 6` and `--window 10000`.

- [ ] **Step 4: Full vol feed in the background**
...
Expected: runs for hours; progress with `tail -3 data/p1/logs_volfeed.txt`; if it is interrupted, start the same command again (finished chunks are skipped). Check `pmset -g batt` before the start: mains power required.

- [ ] **Step 5: Feed check**

Run: `python3 scripts/p1_check_feeds.py`
Expected: every core feed has events in all samples after its start (BTC/ETH from 2 500 000, HYPE from 31 000 000), no replacement candidates; cross-check median |Δ IV| < 0.001 at an SVI age below ~60 s (larger deviations only if the ticker already uses a newer curve; then repeat the run).

- [ ] **Step 6: Fills and classes**

Prerequisite: Task 6 completed.
Run: `python3 -m derive_surface p1 fills`
Expected: `fills` ≈ half of the rows, `unpaired_rows` small and explained, `pair_mismatch` = 0 or explained, `not_settled` reported.

- [ ] **Step 7: Compact after the vol-feed run has finished**

Run: `python3 -m derive_surface p1 compact BTC ETH HYPE`
Expected: monthly files 2024-01 … 2026-09 for BTC/ETH, 2025-10/11 … 2026-09 for HYPE.

- [ ] **Step 8: Write `docs/paper1/DATA_STATUS.md`**

Content: cutoff and retrieval time per dataset; rows per underlying (from `data/p1/tape/MANIFEST.json`) with SHA-256; `rows_in_windows` vs `api_count`; fills, unpaired rows, pair mismatches, not settled; class distribution per underlying (fills and notional) from `fills_summary.json`, but **no markouts**; liquidations reported vs found; maker programmes and active wallets; vault wallets listed vs seen on the tape; vol-feed events per currency and month, disk usage (`du -sh data/p1/*`); result of the feed check; open items.

- [ ] **Step 9: Commit**

```bash
python3 -m pytest -q
git add docs/paper1/DATA_STATUS.md docs/paper1/feed_check.md
...
## Deviations during execution (2026-09-17)

The code in the repository is authoritative; the blocks above show the state before these changes.

- `fulltape`: numeric types forced to float64 (integer strings yielded int64). The API's total count is not additive; instead `recount_days` with re-fetching of mismatching days (`download_tape(max_refetch=2)`), `day_mismatches` and `api_count_full_range` in the manifest. `SETTLE_MS = 60 min` (RFQ maker rows stamped up to 28 min before the fill): the cutoff must lie in the past, and the cache is valid only after this period has elapsed. The CLI writes nothing if days mismatch. Tape start 2023-12-01 (first fill 2023-12-06).
- `api.DeriveClient.call`: unreadable responses (empty body with HTTP 200) are retried instead of crashing.
- `refdata.liquidations`: daily windows in parallel (6 workers), page size 100 with fallback to 20/5 (the first success counts; all sizes return the same auctions, bids incomplete), halving down to 1 h, gaps in the result; the CLI uses `max_retries=3`. An interim union over page sizes was unnecessary and very slow.
- `classify.in_mm_programme`: checks every epoch that contains the fill (overlapping programmes).
- `chainfeeds`: column `block_ts` (push time), `OTHER_FEEDS`, start blocks BTC/ETH 800 000 and HYPE 29 000 000.
- `p1cli.to_ms`: accepts `Z`.
- `scripts/p1_check_feeds.py`: samples from block 800 000 every 250 000; carries the last core forward along, lists gaps and unknown emitters.
- Vol-feed calibration: 5 800 events/s with 4 workers, 43 bytes per event; two processes in parallel trigger HTTP 429 at first, stable afterwards.
- Adversarial review (2 reviewers, 11 findings, 6 confirmed) before the long runs; all 6 incorporated, regression tests added (68 tests).

## Follow-up plans

- **Plan 2 (markouts and inference):** when loading the SVI history, filter on `block <= to_block` of the cutoff (the chunk files can reach beyond the cutoff); `markouts.py` (paths a/b/c/d, three units, cells, exclusions, sweeps), `inference_p1.py` (panel with FE, wild cluster bootstrap, placebo, Lorenz, net edge), Tardis first-of-month data with a size check, document the HYPE listing date. Prerequisite: pre-registration committed, final run at the cutoff 2026-09-30 08:00 UTC (`p1 tape --end 2026-09-30T08:00:00`, `p1 volfeed … --end 2026-09-30T08:00:00`).
- **Plan 3:** Figures T1, T2, 1 to 5, A.
- **Plan 4:** Manuscript (CAS), numbers sheet, two reviewers, SSRN.
