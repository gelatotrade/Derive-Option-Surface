...
- Produces:
  ...
  - `analysis_frame(markouts, funding, horizon=HORIZON, half_spread_bp=1.0) -> DataFrame` (Spalten `y_usd, y_dn, y_vol, hs, as_usd, ne, fe_key, cluster, …`)
    ...
    for cell, (mean, n) in {("BTC", "40-60", "<=2d"): (-1.0, 400), ("BTC", "25-40", "7-30d"): (2.0, 400),
        ...
        for i in range(n):
            rows.append({"currency": cell[0], "delta_bucket": cell[1], "tenor_bucket": cell[2],
                         "cluster": f"w{i % 20}", "ne": mean + rng.normal(0, 0.5)})
    table = inf.cell_table(pd.DataFrame(rows), "ne", min_fills=200, b=199, seed=inf.SEED)
...
def test_analysis_frame_decomposes_the_markout():
    ...
    assert r.ne == pytest.approx(10.0 - 0.4 + 0.1 - r.hedge)
...
def analysis_frame(markouts: pd.DataFrame, funding: pd.DataFrame, horizon: str = HORIZON,
    ...
    f["ne"] = f["y_usd"] - f["fee_maker"].to_numpy() + f["rebate_maker"].to_numpy() - f["hedge"].to_numpy()
...
def run_all(root: Path, out_dir: Path, half_spread_bp: float = 1.0, b: int = B, seed: int = SEED) -> dict:
    ...
    cells = cell_table(frame, "ne", b=b, seed=seed)
    ...
    for name, g in frame.groupby("taker_class"):
        ...
        ci.update({"class": name, "fills": int(len(g)), "mean_dn": float(np.nanmean(g["y_dn"])),
                   "mean_vol": float(np.nanmean(g["y_vol"])), "mean_ne": float(np.nanmean(g["ne"]))})
        classes.append(ci)
    ...
    for bp in PERP_HALF_SPREAD_BP:
        ...
        cells2 = cell_table(f2, "ne", b=min(b, 1999), seed=seed)
...
def main() -> None:
    ...
    lines = [
        ...
        f"{s['fills']:,} Fills; ohne Fill-IV (nur Vol-Einheit betroffen): {s['missing_vol_unit']:,}.".replace(",", " "), "",
        ...
        f"Verlustsumme {de(h1['top10_share']['loss_total'], 0)} USDC über {h1['top10_share']['wallets']:,} Wallets.".replace(",", " "),
        ...
        f"{h2['markout_30m']['n']:,} Fills, {h2['markout_30m']['clusters']} Wallets).".replace(",", " "),
        ...
        f"{h3['did']['n']:,} Fills, {h3['did']['clusters']} Wallets).".replace(",", " "),
    ...
    for row in h4["atm_short"]:
        lines.append(f"- ATM-Kurzläufer {row['currency']} 40–60 Δ, ≤ 2 d: {de(row['mean'], 3)} USDC "
                     f"(90-%-Intervall {de(row['lo'], 3)} bis {de(row['hi'], 3)}, {int(row['fills']):,} Fills)".replace(",", " "))
    ...
    for r in pd.concat([ranked.head(5), ranked.tail(5)]).itertuples():
        lines.append(f"| {r.currency} | {r.delta_bucket} | {r.tenor_bucket} | {int(r.fills):,} | {de(r.mean, 3)} | "
                     f"{de(r.lo, 3)} bis {de(r.hi, 3)} |".replace(",", " "))
    OUT.write_text("\n".join(lines) + "\n")
    print("\n".join(lines))
