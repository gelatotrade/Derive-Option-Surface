...
## Nachtrag 3 (25.09.2026, nach der Erstfassung vom 19.09.2026)

1. **Einheit von Gebühr und Rebate im Netto-Edge:** Im Tape sind `trade_fee` und `expected_rebate` der Maker-Zeile Summen über den ganzen Fill (im Orderbuch ist die Maker-Gebühr überwiegend Menge · min(Rate · Index, 12,5 % · Mark), bei RFQ nicht; in beiden Fällen wächst sie mit der Menge), Markout und Hedge-Kosten dagegen gelten je Kontrakt. Die Definition aus Nachtrag 2, Punkt 2, wird deshalb mit Gebühr und Rebate je Kontrakt umgesetzt: NE_τ = MO_τ − (Maker-Gebühr − Maker-Rebate) / Menge − Hedge-Kosten, in USDC je Kontrakt; der Edge des ganzen Fills ist NE_τ · Menge. Ein Fill ohne positive Menge hat keinen Netto-Edge (im Pilot keiner). Die Erstfassung zog die Summen des Fills vom Markout je Kontrakt ab; das ist nur bei einem Kontrakt dasselbe.
2. **Netto-Edge in bp des Nominals (explorativ, Abbildung F5 und Karte S5):** 10⁴ · NE_τ / Index, gleich dem Edge des Fills geteilt durch sein Nominal (Menge · Index). Die Erstfassung teilte den Wert je Kontrakt durch das Nominal des ganzen Fills. Ebenso wird der Markout als Anteil der Prämie (Abbildungen F1c und F2c) je Kontrakt gebildet: Markout je Kontrakt / Preis.
